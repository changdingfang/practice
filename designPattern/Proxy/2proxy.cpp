...
{
public:
	GamePlayer(string name, IGamePlayer *gamePlayer = NULL)
	{
		if (gamePlayer == NULL)
		{	cout << "不可创建" << endl;	}
		else
...
public:
	GamePlayerProxy(string name)
	{
		//由代理创建真是用户实例
		try
		{	gamePlayer_ = new GamePlayer(name, this);	}
		catch(...)
		{	cout << "出现异常了" << endl;	}
	}
	~GamePlayerProxy() 
...
{
	//产生一个代练
	GamePlayerProxy *gamePlayerProxy = new GamePlayerProxy("dingfang");
