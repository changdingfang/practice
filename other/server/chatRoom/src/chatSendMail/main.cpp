...
int main()
{
	LogTools log_tools("chatSendMail", 5);
	if(0 != log_tools.createLog())
	{
		return -1; 
	}

	TRACE(LOG_ERROR, "chatSendMail tart");

	SendMail sendMail;
	sendMail.readDBConfig();
	int ret = sendMail.initDB();
	if(ret != 0)
	{
	...
	while (FCGX_Accept(&fcgiStdIn, &fcgiStdOut, &fcgiStdErr, &fcgiParamArray) >= 0)
	{
		...
		do {
			...
			fcgiRetData = "";
			fcgiRet = sendMail.messageHandle(fcgibuffer, fcgiRetData);
		} while (0);
