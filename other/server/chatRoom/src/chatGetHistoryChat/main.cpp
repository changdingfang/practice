...
int main()
{
	LogTools log_tools("chatGetHistoryChat", 5);
	if(0 != log_tools.createLog())
	{
		return -1; 
	}

	TRACE(LOG_ERROR, "chatGetHistoryChat tart");

	HistoryChat historyChat;
	historyChat.readDBConfig();
	int ret = historyChat.initDB();
	if(ret != 0)
	{
	...
	while (FCGX_Accept(&fcgiStdIn, &fcgiStdOut, &fcgiStdErr, &fcgiParamArray) >= 0)
	{
		...
		do {
			...
			fcgiRetData = "";
			fcgiRet = historyChat.messageHandle(fcgibuffer, fcgiRetData, callback);
		} while (0);
