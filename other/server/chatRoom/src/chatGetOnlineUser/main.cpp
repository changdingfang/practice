...
int main()
{
	LogTools log_tools("chatGetOnlineUser", 5);
	if(0 != log_tools.createLog())
	{
		return -1; 
	}

	TRACE(LOG_ERROR, "chatGetOnlineUser tart");

	OnlineUser onlineUser;
	onlineUser.readDBConfig();
	int ret = onlineUser.initDB();
	if(ret != 0)
	{
	...
	while (FCGX_Accept(&fcgiStdIn, &fcgiStdOut, &fcgiStdErr, &fcgiParamArray) >= 0)
	{
		...
		do {
			...
			fcgiRetData = "";
			fcgiRet = onlineUser.messageHandle(fcgibuffer, fcgiRetData, callback);
		} while (0);
