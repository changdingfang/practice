...
OnlineUser::OnlineUser() : mip_(""), mport_("")
							   , mdbName_(""), mdbUser_("")
							   , mdbUserPwd_("") 
							   , conn_(NULL)
{
...
int OnlineUser::messageHandle(const string &request, string &response, string &callback)
{
	clean();
	...
	Json::Value jsonData;
	ret = getOnlineUser(jsonData);
	if (0 != ret)
	{
		TRACE(LOG_ERROR, "OnlineUser::messageHandle error, ret: " << ret << ", request: " << request);
		return -4;
	}
...
string OnlineUser::trim(string& s)
{
	int n = 0;
...
void OnlineUser::getParameter(const string &keyWord, const string &query, string &reply)
{
...
int OnlineUser::parseQuery(const string &query)
{
	string buf;
	...
	{
		requestInfo_.pageNum_ = ONLINE_USER_PAGE_NUM;
	}
	buf.clear();
	...
	TRACE(LOG_TRACE, "OnlineUser::parseQuery userIdx: " << requestInfo_.userIdx_
...
int	OnlineUser::getOnlineUser(Json::Value &jsonData)
{
	stringstream sql;

	sql << "select O.user_idx, P.nick_name, P.sex, P.photo from t_online_status as O "
		<< " inner join t_user_profile as P on P.user_idx = O.user_idx where O.room_id = "
		<< requestInfo_.roomId_
		<< " and online_status = "
		<< ON_LINE;
	int begPage = requestInfo_.page_ * requestInfo_.pageNum_ + USER_IDX_BASE;
	int endPage = (requestInfo_.page_ + 1) * requestInfo_.pageNum_ + USER_IDX_BASE - 1;
	sql << " and O.user_idx between " << begPage << " and " << endPage;

	TRACE(LOG_ERROR, "getOnlineUser::getOnlineUser, sql: " << sql.str());
	...
	try
	{
		...
		if(t)
		{
			TRACE(LOG_ERROR, "OnlineUser::getOnlineUser dbcmd(dbproc, sql.str().c_str()) error! sql: " << sql.str());
			reConnectDB();
			return -4;
		}
		else
	...
	{
		TRACE(LOG_ERROR, "database getOnlineUser exception sql: " << sql.str());
		reConnectDB();
		return -3;
	}
