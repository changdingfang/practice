...
int main()
{
	LogTools log_tools("getChatRegister", 5);
	if(0 != log_tools.createLog())
	{
		return -1; 
	}

	TRACE(LOG_ERROR, "getChatRegister tart");

	ChatRegister chatRegister;
	chatRegister.readDBConfig();
	int ret = chatRegister.initDB();
	if(ret != 0)
	{
	...
	while (FCGX_Accept(&fcgiStdIn, &fcgiStdOut, &fcgiStdErr, &fcgiParamArray) >= 0)
	{
		...
		do {
			...
			fcgiRetData = "";
			fcgiRet = chatRegister.messageHandle(fcgibuffer, fcgiRetData, callback);
		} while (0);
