...
int main()
{
	LogTools log_tools("getWxSessionKey", 5);
	if(0 != log_tools.createLog())
	{
		return -1; 
	}

	TRACE(LOG_ERROR, "getWxSessionKey tart");

	WxSessionKey wxSessionKey;
	wxSessionKey.readDBConfig();
	int ret = wxSessionKey.initDB();
	if(ret != 0)
	{
	...
	while (FCGX_Accept(&fcgiStdIn, &fcgiStdOut, &fcgiStdErr, &fcgiParamArray) >= 0)
	{
		...
		do {
			...
			fcgiRetData = "";
			fcgiRet = wxSessionKey.messageHandle(fcgibuffer, fcgiRetData, callback);
		} while (0);
