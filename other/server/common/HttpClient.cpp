...
unsigned char CHttpclient::CharToHex(unsigned char abyChValue)
{
  if (abyChValue > 9) {
...
unsigned char CHttpclient::HexToChar(unsigned char abyChValue)
{
  if (abyChValue >= 'a') {
    return abyChValue - ('a' - 10);
  }
  else if (abyChValue >= 'A') {
    return abyChValue - ('A' - 10);
  }
  else {
    return abyChValue - '0';
...
bool CHttpclient::NeedEncode(char ch, bool bSafe)
{
  string unencode_str = "_.-";
  if (bSafe) {
    unencode_str += "/:=&?#+!$,;'@()*[]";
  }
  int iIndex = 0;
  int iLen = unencode_str.length();
  for (; iIndex < iLen; iIndex++) {
    if (ch == unencode_str[iIndex]) {
      return false;
    }
  }
  return true;
...
string CHttpclient::UrlEncode_safe(const string& astrData, bool bSafe)
{
  std::string lstrData;
  unsigned char ch = 0;
  const char* pszSrc = astrData.c_str();

  for (int32_t i = 0; i < (int32_t)astrData.size(); i++)
  {
    ch = pszSrc[i];
    if ((ch >= '0' && ch <='9')
         || (ch >= 'a' && ch <= 'z')
...
string CHttpclient::UrlDecode(const string& astrData)
{
  ...
  */
  ...
  {  
    ...
    { 
      unsigned char high = FromHex((unsigned char)astrData[++i]);  
      unsigned char low = FromHex((unsigned char)astrData[++i]);  
      strTemp += high*16 + low;  
    } else { 
	  strTemp += astrData[i];
    }	
