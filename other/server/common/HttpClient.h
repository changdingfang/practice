...
public:
    ...
    static std::string UrlEncode_safe(const std::string& astrData, bool bSafe);     //_.-/:=&?#+!$,;'@()*[] 不编码
    static bool NeedEncode(char ch, bool bSafe);     //判断单个字符是否需要编码
    static std::string UrlDecode(const std::string& astrData);
	
	static unsigned char CharToHex(unsigned char abyChValue);
    static unsigned char HexToChar(unsigned char abyChValue);
